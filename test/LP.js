...
contract('LP related test', async ([owner, alice, bob]) => {
  ...
  //   await this.unic.removeFromBlacklist(lpTokenAddress);
  ...
  it('checking that userLPStakes is updating correctly', async () => {
    ...
    await this.unic.approve(this.router.address, ether('5'), { from: owner });
    await this.weth.approve(this.router.address, ether('5'), { from: owner });
    await time.increase(time.duration.days(1)); // startTime + 86400 * 3
    await this.auction.unlockTokens(startTime + 86400 * 2, owner, { from: owner });
    const blockTime = await time.latest();
    await this.router.addLiquidity(this.weth.address, this.unic.address, ether('5'), ether('5'), 0, 0, owner, blockTime + 30, { from: owner });
    const lpTokenAddress = await this.factory.getPair(this.weth.address, this.unic.address);
    this.lpToken = await UniswapV2Pair.at(lpTokenAddress);
