...
contract('AUCTION test', async ([owner, alice, bob]) => {
  ...
  it('should fail to participate, auction hasnt started', async () => {
    const startTime = (await time.latest()) + 86400;
    const cycle2 = await CYCLEToken.new({ from: owner });
    const weth = await WETH.new({ from: owner });
    const factory = await UniswapV2Factory.new(owner, { from: owner });
    const team = web3.eth.accounts.create();
    const router = await UniswapV2Router02.new(factory.address, weth.address, { from: owner });
    const auction = await Auction.new(cycle2.address, router.address, startTime, team.address, { from: owner });
    await cycle2.setAuction(auction.address, { from: owner });
    const pair = await factory.getPair(weth.address, cycle2.address);
    await cycle2.setCYCLEWETHAddress(pair, { from: owner });

    await expectRevert(auction.participate({ from: owner, value: ether('1') }), 'Auction is not started');
  });
